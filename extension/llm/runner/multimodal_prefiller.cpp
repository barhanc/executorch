...
Result<uint64_t> MultimodalPrefiller::prefill(
  ...
  ::executorch::runtime::EValue final_prefill_output;
  ...
  if (enable_parallel_prefill_ || !use_kv_cache_) {
    ...
    final_prefill_output = prefill_outputs[0];

    // Update start_pos, tracking the current cache position.
    start_pos += seq_len;
  } else {
    // Sequential prefill fallback
    auto orig_tensor = encoder_output.toTensor();
    auto embed_dim = orig_tensor.size(2);
    size_t bytes_per_element = orig_tensor.nbytes() / orig_tensor.numel();
    const uint8_t* data_ptr =
        static_cast<const uint8_t*>(orig_tensor.const_data_ptr());

    for (int64_t i = 0; i < seq_len; i++) {
      // Create a tensor for a single token's embedding slice: [1, 1, embed_dim]
      auto slice_tensor = executorch::extension::from_blob(
          const_cast<void*>(static_cast<const void*>(
              data_ptr + i * embed_dim * bytes_per_element)),
          {1, 1, static_cast<executorch::aten::SizesType>(embed_dim)},
          orig_tensor.scalar_type());

      std::vector<int64_t> cache_positions;
      auto cache_pos_res = populate_start_pos_or_cache_position(
          module_, start_pos, cache_positions, 1, kTextModelMethod);
      ET_CHECK_OK_OR_RETURN_ERROR(cache_pos_res.error());
      auto cache_pos_tensor = cache_pos_res.get();

      auto step_result =
          module_->execute(kTextModelMethod, {slice_tensor, cache_pos_tensor});
      if (step_result.error() != ::executorch::runtime::Error::Ok) {
        return step_result.error();
      }
      auto step_outputs = step_result.get();
      if (step_outputs.empty()) {
        ET_LOG(
            Error, "Encoder returned empty results during sequential prefill");
        return ::executorch::runtime::Error::InvalidState;
      }
      final_prefill_output = step_outputs[0];
      start_pos++;
    }
  }

  auto outputs_res = final_prefill_output.toTensor();

...
def _make_int4_weights(E, N, K, group_size, device="cuda"):
    ...
    scale = torch.randn(E, N, K // group_size, device=device, dtype=torch.bfloat16) * 0.01
    return w, scale
...
def _run_eager(hidden_states, w1, w1_scale, w2, w2_scale,
               topk_weights, topk_ids, top_k, num_experts, group_size):
...
def _run_eager_vectorized(hidden_states, w1, w1_scale, w2, w2_scale,
                          topk_weights, topk_ids, top_k, num_experts, group_size):
    ...
    gemm2_out = torch.bmm(
        act.unsqueeze(1), w2_deq[flat_ids].transpose(1, 2)
    ).squeeze(1)
...
def _run_compiled(hidden_states, w1, w1_scale, w2, w2_scale,
                  topk_weights, topk_ids, top_k, num_experts, group_size):
    global _compiled_fn
    if _compiled_fn is None:
        _compiled_fn = torch.compile(_run_eager_vectorized)
    return _compiled_fn(
        hidden_states, w1, w1_scale, w2, w2_scale,
        topk_weights, topk_ids, top_k, num_experts, group_size,
    )


def _run_triton(hidden_states, w1, w1_scale, w2, w2_scale,
                topk_weights, topk_ids, top_k, num_experts, group_size):
    return torch.ops.triton.fused_moe(
        hidden_states, w1, w1_scale, w2, w2_scale,
        topk_weights, topk_ids,
        top_k=top_k, num_experts=num_experts, group_size=group_size,
    )


def _run_triton_batched(hidden_states, w1, w1_scale, w2, w2_scale,
                        topk_weights, topk_ids, top_k, num_experts, group_size):
...
BACKENDS = {
    ...
    "triton_batched": ("Triton batched", _run_triton_batched),
}

# Backends that dequantize all experts (OOM at large M with 256 experts)
_MAY_OOM = {"eager_vec", "compile"}
...
def run_benchmark(
    prompt_lengths,
    num_experts, top_k, hidden_size, intermediate_size, group_size,
    ...
    print()
    print("=" * 100)
    print(f"Fused MoE Benchmark — Qwen3.5-35B-A3B (W4A16)")
    print(f"  Device: {device_name}")
    print(f"  Experts: {num_experts}, Top-K: {top_k}, Hidden: {hidden_size}, "
    ...
    for M in prompt_lengths:
        ...
        common_args = dict(
            ...
            topk_weights=topk_w, topk_ids=topk_i,
            top_k=top_k, num_experts=num_experts, group_size=group_size,
        )
